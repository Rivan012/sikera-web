...
@section('title', 'Post-Test Evaluasi')

@section('content')
<div class="w-full max-w-2xl">
    <div class="mb-6 text-center">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Instrumen Post-Test SIKERA</h1>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Evaluasi akhir setelah mempelajari seluruh modul. Jawaban Anda akan digunakan untuk menghitung skor peningkatan (N-Gain).</p>
        @if($pretest)
        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Skor Pre-Test Anda: {{ $pretest->total_score }}/100</p>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('posttest.submit') }}" x-data="{ currentStep: 0, totalSteps: {{ $questions->count() }}, answers: {} }">
        @csrf
        ...
        <div x-show="currentStep === {{ $index }}" x-transition class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03] sm:p-8">
            <div class="mb-4 flex items-center justify-between">
                ...
                <span class="rounded-full bg-success-50 px-2.5 py-0.5 text-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-400">Post-Test</span>
            </div>
